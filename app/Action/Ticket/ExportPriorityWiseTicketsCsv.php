...
class ExportPriorityWiseTicketsCsv
{
    public function __invoke(string $priority): StreamedResponse
    {
        try {
            ...
            $fileName = 'tickets-' . $priority . '-' . now()->format('Y_m_d_His') . '.csv';

            $response = new StreamedResponse(function() use ($tickets, $priority) {
                $handle = fopen('php://output', 'w');

                fputcsv($handle, ['TICKET REPORT - ' . strtoupper($priority) . ' PRIORITY']);
                fputcsv($handle, ['Total Tickets: ' . $tickets->count()]);
                fputcsv($handle, []);

                fputcsv($handle, ['ID', 'Title', 'Customer', 'Type', 'Status', 'Created', 'Completed']);
            ...
            Log::error('Failed to export priority-wise tickets to CSV: ' . $e->getMessage(), [
                'priority' => $priority,
